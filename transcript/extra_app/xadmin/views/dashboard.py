...
class UserWidgetAdmin(object):
    ...
    list_display = ('widget_type', 'page_id', 'all_user')
    list_filter = ['all_user', 'widget_type', 'page_id']
    list_display_links = ('widget_type',)
    user_fields = ['all_user']
    hidden_menu = True
...
class Dashboard(CommAdminView):
    ...
    def get_context(self):
        new_context = {
            ...
            "?all_user=%s&page_id=%s&_redirect=%s" % (self.user.id, self.get_page_id(), urlquote(self.request.get_full_path()))
        }
